...
///
/// API contract verified live 2026-09-24 (see SPEC.md §Codex). Plans expose a
/// weekly window; windows are labeled by advertised length, never by position.
    ...
    static func windows(from response: Response) -> [LimitWindow] {
        let reported: [Response.Window] =
            [response.rateLimit?.primaryWindow, response.rateLimit?.secondaryWindow]
                .compactMap { $0 }
        return reported.compactMap { window -> LimitWindow? in
